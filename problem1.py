...
def makeSample(args, N=100):
    ...
    def root_eq(D, P):
        # equation F_tlnpdf(D) - P = 0
        D = float(np.atleast_1d(D)[0])
        return F_tlnpdf((ln_Mean, ln_sig, D_Min, D_Max, D, F_DMax, F_DMin)) - P

    # fsolve in place of Secant Method
    d_s = []
    for P in probs:
        # Initial guess: midpoint between D_Min and D_max
        D0 = 0.5 * (D_Min + D_Max)
        D_Solution = fsolve(root_eq, x0=D0, args=(P,))[0]
        d_s.append(D_Solution)
    return d_s
